...
MM_exp <- function(param, sample){
  (mean(sample)- param)^2
}

PM_exp <- function(param, sample){
  (quantile(sample, 0.8)-qexp(0.8, 1/param))^2
}

ML_exp <- function(param, sample){
  -sum(dexp(sample, 1/param, log = TRUE))
}
...
param <- 50
sample_DGP <- rexp(n, 1/param)
mean(sample_DGP)

optim(c(40), MM_exp, method = "L-BFGS-B", sample = sample_DGP)
optim(c(40), PM_exp, method = "L-BFGS-B", sample = sample_DGP)
optim(c(40), ML_exp, method = "L-BFGS-B", sample = sample_DGP)
...
for (i in 1:sim_size){
  sample_DGP <- rexp(n, 1/param)
  MM[i] <- optim(c(40), MM_exp, method = "L-BFGS-B", sample = sample_DGP)$par
  PM[i] <- optim(c(40), PM_exp, method = "L-BFGS-B", sample = sample_DGP)$par
  ML[i] <- optim(c(40), ML_exp, method = "L-BFGS-B", sample = sample_DGP)$par
  setTxtProgressBar(bar, i)
}
bias_MM <- mean(MM)-param
bias_PM <- mean(PM)-param
...
results_exp <- data.frame(method = c("MM", "PM", "ML"),
                          bias = c(bias_MM, bias_PM, bias_ML))
...
results_exp <- cbind(results_exp, MSE = c(MSE_MM, MSE_PM, MSE_ML))
...
bar <- txtProgressBar(0,length(size),style=3)
for (i in 1:length(size)){
  sample_DGP <- rexp(size[i], 1/param)
  MM[i] <- optim(c(40), MM_exp, method = "L-BFGS-B", sample = sample_DGP)$par
  PM[i] <- optim(c(40), PM_exp, method = "L-BFGS-B", sample = sample_DGP)$par
  ML[i] <- optim(c(40), ML_exp, method = "L-BFGS-B", sample = sample_DGP)$par
  setTxtProgressBar(bar, i)
...
plot(size, MM_ex, type = "l", main = "Consistency of MM method",
     xlab = "Sample size", ylab = "Bias")
abline(h = 0, col = "red")
plot(size, PM_ex, type = "l", main = "Consistency of PM method",
     xlab = "Sample size", ylab = "Bias")
abline(h = 0, col = "red")
plot(size, ML_ex, type = "l", main = "Consistency of ML method",
     xlab = "Sample size", ylab = "Bias")
abline(h = 0, col = "red")
...
for (i in 1:sim){
  ...
  expected_pois <- dpois(range, param_pois_cv)*length(sample_test)
  expected_nbinom <- dnbinom(range, param_nbinom_cv[1],
                             param_nbinom_cv[2])*length(sample_test)
...
# Considering the Erlang distribution arises as the exponential distribution,
# from now on, exponential distribution will be analyzed only.

param_exp <- optim(c(1), ML_exp, method = "L-BFGS-B",
                   sample = sev, lower = c(1E-16))$par

#### 3.2.4.1 Graphical comparison --------------------------------------------

range <- seq(-5000, 60000)
exponential <- pexp(range, 1/param_exp)

plot(ecdf(sev), col = "blue", xlab = "Severity",
     main = "Exponential",ylab = "Cumulated probability")
lines(range, exponential, type = "l", col = "red")
legend(x = "bottomright",
       legend=c("Empiric", "Exponential Estimated                            "),
       col=c("blue", "red"), lty=1)
...
bar <- txtProgressBar(0,sim_size,style=3)
for (i in 1:sim_size){
  sample <- rexp(length(sev),1/param_exp)
  ks_exp[i] <- KS_test(sample, pexp,1/param_exp,
                       c(0.001,50000), n=1000)
  cvm_exp[i] <- CVM_test(sample, pexp, dexp, 1/param_exp,
                         c(0.001,50000), n=1000)
  ad_exp[i] <- AD_test(sample, pexp, dexp, 1/param_exp,
                       c(0.001,50000), n=1000)
  setTxtProgressBar(bar, i)
}

hist(ks_exp)
quantile(ks_exp, 0.95)
ks_exp_sev <- KS_test(sev, pexp, 1/param_exp, c(0.001,50000), n=1000)
ks_exp_pvalue <- mean(ks_exp>ks_exp_sev)

hist(cvm_exp)
quantile(cvm_exp, 0.95)
cvm_exp_sev <- CVM_test(sev, pexp, dexp, 1/param_exp,
                        c(0.001,50000), n=1000)
cvm_exp_pvalue <- mean(cvm_exp>cvm_exp_sev)

hist(ad_exp)
quantile(ad_exp, 0.95)
ad_exp_sev <- AD_test(sev,  pexp, dexp, 1/param_exp,
                      c(0.001,50000), n=1000)
ad_exp_pvalue <- mean(ad_exp>ad_exp_sev)
...
ks_gamma_sev <- KS_test(sev, pgamma,c(param_gamma[1], 1/param_gamma[2]),
                        c(0.001,50000), n=1000)
ks_gamma_pvalue <- mean(ks_gamma>ks_gamma_sev)

hist(cvm_gamma)
quantile(cvm_gamma, 0.95)
cvm_gamma_sev <- CVM_test(sev, pgamma, dgamma,
                          c(param_gamma[1], 1/param_gamma[2]),
                          c(0.001,50000), n=1000)
...
ad_gamma_sev <- AD_test(sev, pgamma, dgamma,
                        c(param_gamma[1], 1/param_gamma[2]),
                        c(0.001,50000), n=1000)
...
bar <- txtProgressBar(0,sim,style=3)
for (i in 1:sim){
  sample_train <- sample(sev,n)
  sample_test <- sample_train[1:ceiling(n/4)]
  sample_train <- sample_train[ceiling(n/4+1):n]
  
  param_exp_cv <- optim(c(1000), ML_exp, method = "L-BFGS-B",
                        sample = sample_train, lower = c(0.001,0.001))$par
  param_gamma_cv <- optim(c(0.8,1000), ML_gamma, method = "L-BFGS-B",
                          sample = sample_train, lower = c(0.001,0.001))$par
  
  dx <- 1000
  range <- seq(0,60000,dx)
  observed <- c()
  for (j in 1:length(range)-1) observed[j] <- sum(sample_test<range[j+1]&
                                                    sample_test>=range[j])
  expected_exp <- dexp(range[-length(range)]+dx/2, 1/param_exp_cv)*dx*n/4
  expected_gamma <- dgamma(range[-length(range)]+dx/2, param_gamma_cv[1],
                           scale = param_gamma_cv[2])*dx*n/4
  
  MSE_exp[i] <- sum((observed-expected_exp)^2)/length(range)
  MSE_gamma[i] <- sum((observed-expected_gamma)^2)/length(range)
  setTxtProgressBar(bar, i)
  
}

cv_expected_MSE_exp <- mean(MSE_exp)
cv_expected_MSE_gamma <- mean(MSE_gamma)

cv_sd_MSE_exp <- sd(MSE_exp)
cv_ad_MSE_gamma <- sd(MSE_gamma)

### 3.2.7 Results summary -------------------------------------------------------

results_sev <- data.frame(distribution = c("Lognormal", "Exponential", "Gamma"),
                          KS = c(ks_lnorm_sev,
                                 ks_exp_sev,
                                 ks_gamma_sev),
                          ...
                          AD_pvalue = c(ad_lnorm_pvalue,
                                        ad_exp_pvalue,
                                        ad_gamma_pvalue),
                          CV_expect_MSE = c(NA, cv_expected_MSE_exp,
                                            cv_expected_MSE_gamma),
                          CV_sd_MSE = c(NA, cv_sd_MSE_exp,cv_ad_MSE_gamma))
...
cost_nl_single <- c()
for (i in 1:sim){
  cost_nl_single[i] <- sum(rexp(n[i], 1/param_exp))
}
...
for (i in 1:sim){
  ...
  cost_nl[i] <- sum(rexp(n, 1/param_exp))
  setTxtProgressBar(bar, i)
}
...
hist(cost_nl, breaks = 100, probability = TRUE, xlab = "Cost (Millions)",
     main = "All car policies cost distribution", xaxt="n")
lines(x, dnorm(x, mean(cost_nl), sd(cost_nl)), col="blue", lwd = 2)
axis(1, at = seq(2.5E8, 2.8E8, 1E6), labels = seq(2.5E2, 2.8E2, 1))
...
write_xlsx(results_exp,"Results/results_exp.xlsx")
...
write_xlsx(results_sev,"Results/results_sev_test.xlsx")
